...
-- MySQL Workbench용
-- 
-- 실행: 전체 선택 후 Ctrl+Shift+Enter (또는 ⚡ 버튼)
-- ============================================

-- 데이터베이스 선택
USE aller;

-- MySQL Workbench 안전 모드 해제 (DELETE 허용)
SET SQL_SAFE_UPDATES = 0;
SET FOREIGN_KEY_CHECKS = 0;

-- ============================================
-- 0. 기존 데이터 정리 (TRUNCATE = 즉시 삭제)
-- ============================================
TRUNCATE TABLE event_logs;
TRUNCATE TABLE search_queries;
TRUNCATE TABLE recommendation_feedback;
...
SELECT
    UUID() AS session_id,
    -- 로그인 필수 서비스: 모든 세션에 user_id 존재 (1~500번 유저)
